...
test.skip("early settlement: what is owed, what the penalty costs and what the interest saving is", async (t) => {
...
test.skip("a partial overpayment re-amortises the rest and the saving is the difference in interest", async (t) => {
  const { c, id } = await withLoan(t, {}, { key: proKey() });
...
test.skip("the journal debits interest and liability, credits cash, and the expense payload carries the interest alone", async (t) => {
...
test.skip("the list and the report: outstanding, next payment date and the interest for the year", async (t) => {
  const { c, id } = await withLoan(t, {}, { key: proKey() });
